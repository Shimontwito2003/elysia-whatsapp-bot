...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; background:#111; color:#fff; padding:14px; margin:0; }
    h1 { color:#d6b56d; font-size:26px; text-align:center; }
    .stats { display:grid; grid-template-columns:repeat(2,1fr); gap:8px; margin-bottom:14px; }
    .stat { background:#1c1c1c; border:1px solid #333; border-radius:14px; padding:14px; text-align:center; }
    .stat strong { display:block; color:#d6b56d; font-size:24px; }
    .filters,.customer { background:#1c1c1c; border:1px solid #333; border-radius:14px; padding:14px; margin-bottom:14px; }
    input,select,textarea { width:100%; border-radius:10px; border:none; padding:12px; margin-top:8px; box-sizing:border-box; font-family:Arial; }
    button,.wa { display:block; width:100%; box-sizing:border-box; margin-top:8px; padding:12px; background:#d6b56d; color:#111; border:none; border-radius:10px; font-weight:bold; text-align:center; text-decoration:none; }
    .top { display:flex; justify-content:space-between; gap:8px; align-items:center; margin-bottom:10px; }
    .phone { font-weight:bold; color:#d6b56d; direction:ltr; font-size:20px; }
    .badge { background:#333; padding:6px 10px; border-radius:20px; font-size:12px; display:inline-block; margin:2px; }
    .msg { background:#2a2a2a; padding:10px; border-radius:10px; margin:7px 0; line-height:1.5; }
    .customer-msg { border-right:4px solid #d6b56d; }
    .business-msg { border-right:4px solid #7da7ff; }
    .note { background:#161616; border:1px dashed #555; padding:10px; border-radius:10px; margin-top:8px; white-space:pre-wrap; }
...
async function handleSelection(to, selectedId) {
  switch (selectedId) {
    case "menu_collections": return sendCollectionsMenu(to);
    case "menu_service": return sendServiceMenu(to);
    case "menu_help_choose": return sendChooseHelp(to);
    case "rings": return sendText(to, `טבעות Elysia 💍\n${SITE_URL}/rings`);
    case "necklaces": return sendText(to, `שרשראות Elysia ✨\n${SITE_URL}/necklaces`);
    case "earrings": return sendText(to, `עגילים Elysia 🤍\n${SITE_URL}/earrings`);
    case "bracelets": return sendText(to, `צמידים Elysia\n${SITE_URL}/bracelets`);
    case "all_collection": return sendText(to, `לצפייה במבחר המלא:\n${SITE_URL}/search`);
    case "gifts": return sendText(to, `מתנות תכשיטים 🎁\n${SITE_URL}/gifts`);
    case "size_guide": return sendText(to, `מדריך מידות 📏\n${SITE_URL}/size-guide`);
    case "personal_service": return sendText(to, "ייעוץ אישי 🤍\nשלח/י לנו למי התכשיט מיועד, תקציב, סגנון והאם זו מתנה.");
    case "shipping": return sendText(to, "משלוחים והזמנות 🚚\nלשאלה על הזמנה קיימת, שלח/י מספר הזמנה.");
    case "returns": return sendText(to, `החלפות והחזרות\n${SITE_URL}/terms`);
    case "human": return sendText(to, "נציג אנושי יחזור אליך בהקדם 🤍\nאפשר לכתוב כאן את השאלה.");
    case "about": return sendText(to, `אודות Elysia\n${SITE_URL}/about`);
    default: return sendMainMenu(to);
  }
}
...
async function sendMainMenu(to) {
  await fetch(`https://graph.facebook.com/v20.0/${PHONE_NUMBER_ID}/messages`, {
    ...
    body: JSON.stringify({
      ...
      interactive: {
        type: "button",
        body: { text: "ברוכים הבאים ל־Elysia Jewellery ✨\nאיך אפשר לעזור?" },
        action: {
          buttons: [
